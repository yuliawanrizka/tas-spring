...
/*  Created On : 13-Sep-2017 10:06:41 				*/
...
IF EXISTS (SELECT 1 FROM dbo.sysobjects WHERE id = object_id(N'[FK_JWT_EmployeeData]') AND OBJECTPROPERTY(id, N'IsForeignKey') = 1) 
ALTER TABLE [Token] DROP CONSTRAINT [FK_JWT_EmployeeData]
GO
...
CREATE NONCLUSTERED INDEX [IXFK_JWT_EmployeeData] 
...
CREATE NONCLUSTERED INDEX [IXFK_TrainingAchievement_CourseName] 
 ON [TrainingAchievement] ([CourseID] ASC)
GO
...
ALTER TABLE [Token] ADD CONSTRAINT [FK_JWT_EmployeeData]
...
ALTER TABLE [TrainingAchievement] ADD CONSTRAINT [FK_TrainingAchievement_CourseName]
	FOREIGN KEY ([CourseID]) REFERENCES [CourseName] ([CourseNameID]) ON DELETE No Action ON UPDATE No Action
GO
